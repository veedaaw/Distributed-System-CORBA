/* This file was generated by SableCC (http://www.sablecc.org/). */

package net.sf.eclipsecorba.idl.node;

import java.util.*;
import net.sf.eclipsecorba.idl.analysis.*;

public final class ATypeId extends PTypeId
{
    private TTypeid _typeid_;
    private TIdentifier _identifier_;
    private TStringLiteral _stringLiteral_;

    public ATypeId()
    {
    }

    public ATypeId(
        TTypeid _typeid_,
        TIdentifier _identifier_,
        TStringLiteral _stringLiteral_)
    {
        setTypeid(_typeid_);

        setIdentifier(_identifier_);

        setStringLiteral(_stringLiteral_);

    }
    public Object clone()
    {
        return new ATypeId(
            (TTypeid) cloneNode(_typeid_),
            (TIdentifier) cloneNode(_identifier_),
            (TStringLiteral) cloneNode(_stringLiteral_));
    }

    public void apply(Switch sw)
    {
        ((Analysis) sw).caseATypeId(this);
    }

    public TTypeid getTypeid()
    {
        return _typeid_;
    }

    public void setTypeid(TTypeid node)
    {
        if(_typeid_ != null)
        {
            _typeid_.parent(null);
        }

        if(node != null)
        {
            if(node.parent() != null)
            {
                node.parent().removeChild(node);
            }

            node.parent(this);
        }

        _typeid_ = node;
    }

    public TIdentifier getIdentifier()
    {
        return _identifier_;
    }

    public void setIdentifier(TIdentifier node)
    {
        if(_identifier_ != null)
        {
            _identifier_.parent(null);
        }

        if(node != null)
        {
            if(node.parent() != null)
            {
                node.parent().removeChild(node);
            }

            node.parent(this);
        }

        _identifier_ = node;
    }

    public TStringLiteral getStringLiteral()
    {
        return _stringLiteral_;
    }

    public void setStringLiteral(TStringLiteral node)
    {
        if(_stringLiteral_ != null)
        {
            _stringLiteral_.parent(null);
        }

        if(node != null)
        {
            if(node.parent() != null)
            {
                node.parent().removeChild(node);
            }

            node.parent(this);
        }

        _stringLiteral_ = node;
    }

    public String toString()
    {
        return ""
            + toString(_typeid_)
            + toString(_identifier_)
            + toString(_stringLiteral_);
    }

    void removeChild(Node child)
    {
        if(_typeid_ == child)
        {
            _typeid_ = null;
            return;
        }

        if(_identifier_ == child)
        {
            _identifier_ = null;
            return;
        }

        if(_stringLiteral_ == child)
        {
            _stringLiteral_ = null;
            return;
        }

    }

    void replaceChild(Node oldChild, Node newChild)
    {
        if(_typeid_ == oldChild)
        {
            setTypeid((TTypeid) newChild);
            return;
        }

        if(_identifier_ == oldChild)
        {
            setIdentifier((TIdentifier) newChild);
            return;
        }

        if(_stringLiteral_ == oldChild)
        {
            setStringLiteral((TStringLiteral) newChild);
            return;
        }

    }
}
